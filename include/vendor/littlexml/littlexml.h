/*
 * Original file by @jonahisadev on GitHub
 * 
 * Modifications (by @what-is-a-git on GitHub):
 * 
 * include a extern "C" for cpp
 * remove the writing functions
 * change xmldocument_free to properly free the whole document by looping through each node in it to prevent memory leaks! (doesn't apply to node lists but still :])
 * switch xmlnode_attr functions to using const char * for cpp's lack of string literal to const char * support :)
 * 
 * :]
 */ 
...
XMLAttribute* XMLNode_attr(XMLNode* node, const char* key);
